...
def _layout_items(vehicle: dict[str, object], load_items: Iterable[dict[str, object]]) -> tuple[list[dict[str, object]], int]:
    ...
    # 층별 슬롯 카운터 — 각 층 내에서 독립적으로 위치 계산
    layer_slot_counters: dict[int, int] = {}
    ...
    for index, item in enumerate(items):
        ...
        slot_index = layer_slot_counters.get(layer, 0)
        layer_slot_counters[layer] = slot_index + 1
...
def _build_floor_plan_svg(vehicle: dict[str, object], items: list[dict[str, object]]) -> str:
    ...
    floor_items = [item for item in items if int(item["layer"]) == 1]
    ...
    bottom_count = len(floor_items)
    parts.append(
        f'<text x="460" y="270" fill="#64748b" font-size="11" text-anchor="middle">1층 {bottom_count}팔레트</text>'
    )
    parts.append("</svg>")
    return "".join(parts)
...
def _build_side_view_svg(vehicle: dict[str, object], items: list[dict[str, object]]) -> str:
    ...
    layer_height = 72
    bottom_items = [item for item in items if int(item["layer"]) == 1]
    top_items = [item for item in items if int(item["layer"]) > 1]
    parts = [
        ...
        f'<text x="48" y="24" fill="#1e293b" font-size="16" font-weight="700">측면도 — 1층 {len(bottom_items)}팔레트 / 2층 {len(top_items)}팔레트</text>',
    ...
    for layer in range(layer_count):
        ...
        if layer == 0:
            parts.append(f'<text x="56" y="{label_y:.1f}" fill="#1d4ed8" font-size="13" font-weight="700">{label}</text>')
        else:
            parts.append(f'<text x="56" y="{label_y:.1f}" fill="#60a5fa" font-size="13" font-weight="700">{label}</text>')
...
def _build_weight_map_svg(vehicle: dict[str, object], items: list[dict[str, object]]) -> str:
    ...
    bubble_centers = [(120, 92), (300, 92), (120, 196), (300, 196)]
    ...
    for index, zone_name in enumerate(zone_names):
        ...
        parts.append(f'<circle cx="{cx}" cy="{cy}" r="{radius:.1f}" fill="{fill}" stroke="#1e3a8a" stroke-width="2"/>')
        parts.append(f'<text x="{cx}" y="{cy - 10:.1f}" fill="white" font-size="13" font-weight="700" text-anchor="middle">{zone_name}</text>')
        parts.append(f'<text x="{cx}" y="{cy + 12:.1f}" fill="white" font-size="22" font-weight="700" text-anchor="middle">{pct}%</text>')
        parts.append(f'<text x="{cx}" y="{cy + 30:.1f}" fill="rgba(255,255,255,.92)" font-size="11" text-anchor="middle">{zone_weights[index]:.0f}kg</text>')
    parts.append(f'<text x="64" y="250" fill="#475569" font-size="12">전후편차 {front_rear_deviation:.1f}%</text>')
    parts.append(f'<text x="176" y="250" fill="#475569" font-size="12">좌우편차 {left_right_deviation:.1f}%</text>')
    parts.append(f'<rect x="300" y="236" width="74" height="24" rx="12" fill="{risk_color}"/>')
    parts.append(f'<text x="337" y="252" fill="white" font-size="12" text-anchor="middle" font-weight="700">{risk_level}</text>')
    parts.append("</svg>")
    return "".join(parts)


def build_vehicle_visualization(vehicle: dict[str, object], load_result: dict[str, object]) -> dict[str, object]:
    placements = load_result.get("placements", [])
    items_for_svg, _ = _layout_items(vehicle, placements)
    
    grid_info = load_result.get("floor_grid", {"cols": 6, "rows": 2})
    # Force rows=2 for standard 25t layout if it comes as 1
    if grid_info.get("rows", 0) <= 1:
        grid_info["rows"] = 2
        grid_info["cols"] = 6
        
    slots_per_layer = grid_info["cols"] * grid_info["rows"]
    
    packed_items = []
    for idx, p in enumerate(placements):
        floor_idx = idx % slots_per_layer
        risk_lvl = 0
        p_risk = p.get("pallet_risk", "Safe")
        if p_risk in ["Critical", "Danger"]: risk_lvl = 2
        elif p_risk == "Caution": risk_lvl = 1

        packed_items.append({
            "id": idx + 1,
            "col": floor_idx // grid_info["rows"],
            "row": floor_idx % grid_info["rows"],
            "layer": 1 if p.get("vertical_zone") == "bottom" else 2,
            "qty": int(p.get("qty", 1)),
            "weight": float(p.get("weight_kg", 0.0)),
            "risk_level": risk_lvl,
            "material_name": str(p.get("material_name") or p.get("material_key", "")),
            "unit": str(p.get("unit_type", "pallet"))
        })

    return {
        "packed_items": packed_items,
        "items": packed_items, # Test compatibility
        "vehicle": {
            "name": vehicle.get("vehicle_name", ""),
            "length": float(vehicle.get("cargo_length_mm", 0.0)),
            "width": float(vehicle.get("cargo_width_mm", 0.0)),
            "height": float(vehicle.get("cargo_height_mm", 0.0)),
            "max_weight": float(vehicle.get("max_weight_kg", 0.0)),
            "cols": grid_info["cols"],
            "rows": grid_info["rows"]
        },
        "summary": {
            "total_weight": float(load_result.get("total_weight_kg", 0.0)),
            "utilization": float(load_result.get("weight_ratio_pct", 0.0)),
            "cog_fr": float(load_result.get("front_rear_deviation_pct", 0.0)),
            "cog_lr": float(load_result.get("left_right_deviation_pct", 0.0)),
            "risk_count": len(load_result.get("risks", []))
        },
        "floor_plan": _build_floor_plan_svg(vehicle, items_for_svg),
        "side_view": _build_side_view_svg(vehicle, items_for_svg),
        "weight_map": _build_weight_map_svg(vehicle, items_for_svg),
    }
